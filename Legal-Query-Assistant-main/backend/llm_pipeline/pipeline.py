    ...
    def _generate_response(self, query_ctx: QueryContext) -> tuple[str, dict]:
        ...
        try:
            ...
            if query_ctx.context_quality == ContextQuality.NO_CONTEXT:
                ...
                excerpts = []
                for i, ctx in enumerate(query_ctx.contexts[:3], 1):
                    text = " ".join(ctx.text.split())
                    if len(text) > 500:
                        text = text[:500].rsplit(" ", 1)[0] + "..."
                    source = ctx.format_citation()
                    excerpts.append(f"{i}. {text}\nSource: {source}")

                fallback = (
                    "I found relevant document content, but the Groq API connection "
                    "failed while generating the final answer.\n\n"
                    "Relevant excerpts:\n" + "\n\n".join(excerpts)
                )
